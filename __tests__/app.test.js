...
describe('/api/articles/:article_id', () => {
  ...
  describe('HAPPY PATH PATCH /api/articles/:article_id', () => {
    ...
    test('200 OK - Should return a 200 status code if succesful', () => {
      return request(app).patch('/api/articles/2').send(newVote).expect(200);
    });
    ...
    describe('SAD PATH queries /api/articles ', () => {
      ...
      test('Should be 400 bad request for invalid order by', () => {
        return request(app)
          ...
          .then((response) => {
            const message = {
              msg: 'Bad request - invalid order by, please use asc or desc',
            };
            expect(response.body).toEqual(message);
          });
      });
  ...
  describe('204 DELETE HAPPY PATH /api/comments/:comment_id', () => {
    test('/api/comments/:comment_id - should respond 204 & delete restaurant', () => {
      return request(app)
        ...
        .then(() => {
          return db.query(
            `SELECT comment_id, votes, created_at, author, body FROM comments WHERE article_id = 6`
          );
        })
        .then((response) => {
...
describe('GET: HAPPY PATH /api', () => {
  test('status 200: should return a JSON object of all api endpoints', () => {
    return request(app)
      .get('/api')
      .expect(200)
      .then((response) => {
        expect(typeof response._body.endPoints).toBe('object');
      });
  });
});
